...
**الحالة:** ✅ منفذة بالكامل (2026-09-20) — 61 اختبار passing، ruff نظيف
...
- [x] حذف `run_command` و`read_file` و`write_file` من `OSAgent` ومن أي مسار يصل إليه تيليجرام.
- [x] `TELEGRAM_CHAT_ID` إلزامي: إن فُضيت القائمة → البوت يرفض الجميع ويحذّر في اللوج.
- [x] إزالة رابط الشيت الحقيقي من `config.py` وchat IDs الحقيقية من `.env.example`.
- [x] إضافة `TIMEZONE` (افتراضي `Asia/Riyadh`) وتصحيح كل `datetime.now()` عبر `zoneinfo`.
- [x] توحيد النشر: حذف `Procfile` والإبقاء على `Dockerfile` + `railway.json`.

### المرحلة 2 — 📊 طبقة البيانات: قراءة وكتابة
- [x] استبدال تنزيل XLSX بـ **gspread** عبر Service Account (متغير `GOOGLE_SERVICE_ACCOUNT_JSON` — يقبل JSON أو Base64).
- [x] كاش في الذاكرة (TTL ~60 ثانية) لتقليل استدعاءات API.
- [x] المواعيد تنتقل إلى تاب **Appointments** في الشيت (تنتهي مشكلة الفقدان مع إعادة النشر).
- [x] تاب **Tasks** للمهام الدراسية.
- [x] أوامر الكتابة: `/log <كود> <دقائق>` ← SessionLog (تاب مستقل بمخطط نظيف؛ DailyLog يبقى بيد الطالب)، `/done <مهمة>` ← Tasks، `/appt` ← Appointments.
- [x] عدم توفر Service Account → البوت يعمل بوضع قراءة فقط مع رسالة واضحة (لا انهيار).

### المرحلة 3 — 🤖 عقل الوكيل: Function Calling
- [x] تعريف الأدوات بصيغة JSON Schema: `log_study`, `mark_done`, `add_appointment`, `get_summary`, `get_risk`, `send_parent_report`, `ask_tutor`.
- [x] تنفيذ عبر Bedrock Converse API (tool use) + Gemini function calling، بنفس مخطط الأدوات.
- [x] الإبقاء على الأوامر المباشرة (`/summary`, `/risk`, ...) كمسار سريع واحتياطي عند تعطل الـ AI.
- [x] تحقق صارم من مخرجات الأداة قبل التنفيذ (whitelist، لا نص حر يمر للنظام).
- [x] ذاكرة جلسة قصيرة لكل chat (آخر ~10 رسائل، في الذاكرة).

### المرحلة 4 — ⏰ الجدولة والاستباقية
- [x] APScheduler داخل عملية البوت.
- [x] تقرير يومي 21:00 توقيت الرياض → تيليجرام للطالب والوالد (البريد اختياري عبر `/sendreport` فقط).
- [x] تنبيه قبل كل موعد من تاب Appointments بساعة (قابل للضبط).
- [x] تنبيه ذكي: إذا لم يُسجل الطالب أي جلسة حتى 20:00 → تذكير لطيف.
- [x] `/status` يعرض آخر تشغيل للمجدول وآخر مزامنة للشيت.

### المرحلة 5 — 🧪 الجودة والنشر
- [x] اختبارات pytest لطبقة الشيت (بملفات وهمية) والأدوات والتقارير.
- [x] GitHub Actions: `pytest` + `ruff` على كل push.
- [x] تحديث README ليطابق النظام الجديد.
- [x] مراجعة: `boto3` يبقى (قرار Bedrock+Gemini) مع استيراد كسول، وإعادة تسمية `BedrockService` إلى `AIService`.
